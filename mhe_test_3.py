...
for i in range(n - N+1):
    ...
    c3 = k_rk4 * sum(sum1((x_vars[j] - phi(x_vars[j-1], force_vars[j-1]))**2) for j in range(1,N))/sigma_x/N
    ...
    opti.minimize(c)
    opti.solver('ipopt', {"ipopt.print_level":0, "print_time": False, 'ipopt.max_iter':10000})
    if i != 0:
    ...
    arrs.append([(opti.value(x), opti.value(f)) for x, f in zip(x_vars, force_vars)])
    print(i,":")
